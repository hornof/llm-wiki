...
- [[openteams]] — [[travis-oliphant|Travis Oliphant]]'s (NumPy/Anaconda) *"Distributed AI Economy"* ([[openteams-distributed-ai-economy-whitepaper-2026-06]]): the same org-scale rebuild from the **sovereignty/ownership** angle — an Intelligence Hub + **Frames** (portable, governed org-context artifacts) + Cogs + Ops marketplace. Where 8090 pitches a control plane and this concept describes Block's world-model, OpenTeams supplies the open-standard + [[reverse-information-paradox|own-your-learning-loop]] framing. Vision-stage single source; the durable idea is *organizational context as a first-class, org-owned artifact*.
...
**Working autonomous-company build with a governance layer (2026-08-08, [[practical-systems-autonomous-company-dashclaw-2026-08-08]])**: a solo builder (r/ClaudeCode) runs an **11-step "company loop"** on a FastAPI backend with 8 agent roles (CEO/researcher/brainstormer/builder/QA/salesperson/marketer/finance): read market signals → rank opportunities → pick a product → write a one-page `GOAL.md` → dispatch a **headless [[claude-code|Claude Code]]** build (`claude -p "/supergoal Build @GOAL.md … no human present, do not stop until finished" --model claude-fable-5 --max-turns 200 --permission-mode bypassPermissions`, 120-min wall-clock + 10-sec-polled kill switch) → QA → draft outreach → close with a P&L line. It built *deposit-watchdog* (51-state statute table, 44 tests) in **71 minutes untouched**. The load-bearing part is **DashClaw**, a governance control-plane: every action carries a **risk score**, and org policies gate it — low-risk actions log, but `outreach_send` / `charge_customer` **park as `pending_approval`** until the human clicks approve. *"Agents that can email real people or spend money need a control plane, not vibes."* **Honest numbers**: ~$0.20/cycle, $2.73 total spend, **$0.00 revenue** — *"whether anyone pays for what it builds is the actual experiment."* Best design lesson: a policy cleanup silently deleted the `require_approval` rule, so drafts queued but no approval inbox ever filled — now a **hard assertion** (a send-gate action that doesn't park raises and pauses the cycle): *"governance bugs should be loud."* The [[loop-engineering|loop]] + human-gated-outbound pattern is the credible-but-honest counterweight to the $1M-solo-founder hype above.
