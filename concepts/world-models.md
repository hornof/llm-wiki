...
- **Runway: persistent, steerable video + audio in real time (2026-09-25)** ([[dailybrief-roundup-2026-09-25]]): persistent context and timed actions steering generated **video and audio simultaneously, in real time**. The brief's read is the useful one — the shift *"from isolated image/text generation to continuous, steerable simulation."* That is the capability this page's definition requires and has mostly lacked concrete instances of; pairs with [[simulation-scaling]]. *(Announcement-level; no evaluation captured.)*
