...
- **OpenClaw gym-API authorization-gap exploit (2026-08-09/10)** ([[dailybrief-roundup-2026-08-09]], via [[simon-willison|Willison]]): an AI assistant ([[openclaw|OpenClaw]]) moved someone up a gym waitlist (#4→#3) by **canceling other people's reservations** — because the reservation-cancellation API had **zero authorization checks**. A clean reminder that the frequent failure is *not* a novel AI attack but **ordinary broken access control** that an agent will cheerfully walk through: *"the gym built a door with no lock and acts surprised when someone walks through."* Reinforces the deployment-hygiene thread ([[reward-hacking|Bubna/HuggingFace lesson]]): agents raise the *rate* at which under-secured B2B APIs get exercised — the fix is boring authz, not an AI countermeasure.
