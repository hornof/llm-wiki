...
- **Team-Scale Instantiation: Team OS Pattern (Stulberg, Killeen, Meyer, Vellotti, May 2026)**: Three-folder repo (`.claude/` + `product-development/` + `team/`) with a minimal CLAUDE.md at root containing doc-index + team roster + Slack channels. Function-specific subfolders (`pm/`, `eng/`, `design/`, `data/`, etc.) each owned by a function lead who checks in canonical context. Sub-CLAUDE.mds load progressively as natural-language queries reference them. Demoed at 3% of context-window per query; new engineer answered a 3-month-old customer-decision question in 15 seconds without paging the PM. **Four independent practitioner instantiations converged on the same architecture** without coordination ([[stahlberg-team-os-aakash-2026-05|Stulberg/DoorDash, Killeen/Pendo, Meyer/Google, Vellotti solo]]). Aakash Gupta's followup essay: *"That convergence is what convinced me to write this."* Quantified ROI: per-person context-question cost ≈ 8+ hrs/week (10 questions/day × 10 minutes each including the Slack-ping-and-wait roundtrip). Maps cleanly to the three Ashwingop layers — function-folder docs (factual memory), CLAUDE.md doc-index + Slack-handle bindings (context graph), Slack/MCP send commands (action coordination). First wiki capture of the **team-scope** instantiation; sits between [[milesdeutscher-claude-personal-cfo-2026-05-14|personal scale]] and [[ashwingop-company-brain-part-1|organizational scale]]. Cross-confirms the [[aakashgupta-doordash-company-brain-2026-05-10|earlier DoorDash decision-context-via-AI capture]] — Stulberg is now identified as the practitioner behind it. — [[stahlberg-team-os-aakash-2026-05]]
...
- **Comparative teardown of 9+ "company brains" — the four shared functions (Femke Plantinga / Slite, 2026-08-27)** ([[femke-plantinga-company-brain-teardown-2026-08-27]]): the first cross-implementation **field-map** of the category. Every one does the same four things — **getting signals · remembering · dreaming & pruning · speaking & searching** — which extends this page's three-layer model (factual memory / context-graph / action-coordination) by promoting **pruning/forgetting to a first-class function** (the *"memory-curation problem named and unsolved"* above). The systems, by archetype:
  - **Git-repo-as-substrate**: **GBrain** ([[garry-tan|Garry Tan]]'s open-source personal brain — email/calendar → git, nightly re-link + staleness flag), **Sylph** (content brain; agents draft, humans publish, agent learns from your edits), **DIY (Claude Code + git)** — *"markdown in the repo, grep instead of search, PRs as the only thing keeping it honest"* = literally this wiki's [[llm-wiki-pattern]] / [[claude-md-pattern]] — and **Gorgias Cortex** (8-person AI team, **12,000 markdown nodes in GitHub; nightly, the questions it got wrong become PRs that fix it** — the [[loop-engineering|compound-engineering / self-improving outer loop]] at org scale).
  - **Memory libraries / agent memory**: **[[mem0]]** (explicit-store, freshness-ranked), **Letta** (MemGPT lineage; the agent decides what to keep and **a second agent tidies memory in the background** — pruning as a dedicated agent).
  - **Temporal knowledge graph**: **Zep / Graphiti** — *"a knowledge graph with a clock in it"*: a changed fact gets an **end-date instead of being overwritten** (ask *"what was true last March"*) — the [[graph-engineering|knowledge-graph-as-memory]] pattern made bi-temporal, cousin to the [[graph-engineering|"memory of why"]] assumption-provenance schema.
  - **Human-gated vertical brains**: **Pletor** (brand brain; rules move only on human sign-off), **Slite Agent** (watches ~20 tools for staleness, routes diffs to page-owners, **nothing changes without approval**).
  
  Pull-quote (comment thread): *"everyone builds the remembering part, nobody wants to own the forgetting part."* Convergent patterns across all nine: **git/markdown substrate + human-approval gates on writes + background pruning + temporal state.** *(Vendor-authored (Slite); "9+" self-selected; promotional ebook tie-in — the taxonomy and named systems are the trackable part.)* **Create-candidates**: `letta`, `zep` (2nd surface would justify pages); `femke-plantinga`/`slite` noted, single promotional surface.
