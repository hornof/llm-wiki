...
- **"Mechanistic Tomography: Designed Measurement for Control-Oriented Interpretability"** (arXiv 2608.19338, [[dailybrief-roundup-2026-08-21]]) — reframes the interpretability toolkit (**activation patching, gradients, Hessians**) as a single **measurement problem**: each technique is designed measurement under different assumptions, so the binding constraint shifts from *"which method"* to *"what can I access / what's my intervention + compute budget."* A rigor/unification advance aimed at **control-oriented** interpretability (measure in order to steer), complementary to the causal + formal-verification directions on this page. *(arXiv; primary not deeply fetched.)*
